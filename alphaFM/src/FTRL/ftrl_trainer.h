...
private:
    void train(int y, double weight, const vector<pair<string, double> >& x);
...
void ftrl_trainer<T>::run_task(vector<string>& dataBuffer)
{
    for(size_t i = 0; i < dataBuffer.size(); ++i)
    {
        ...
        train(sample.y, sample.weight, sample.x);
    }
}
...
//输入一个样本，更新参数
template<typename T>
void ftrl_trainer<T>::train(int y, double sample_weight, const vector<pair<string, double> >& x)
{
    ...
    double mult = y * (1 / (1 + exp(-p * y)) - 1) * sample_weight;
    //update w_n, w_z
    for(int i = 0; i <= xLen; ++i)
    {
        ftrl_model_unit<T>& mu = i < xLen ? *(theta[i]) : *thetaBias;
